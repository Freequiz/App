...
class _ProfileInfoState extends State<ProfileInfo> {
  ...
  Widget build(BuildContext context) {
    futureMapInfo = httpGetProfile(Profile.sessionToken);
              ...
              children: [
                ...
                Align(
                  child: TextButton(
                    ...
                    onPressed: () async {
                      final status =
                          await httpDeleteSession(Profile.sessionToken);
                      if (status == "Success") {
  ...
  changePassword() async {
    final Map map = await httpPatchPassword(
        Profile.sessionToken, newPassword.text, newPasswordConfirmation.text);
    if (map["success"] == true) {
      Profile.sessionToken = "";
      Profile.date = 0;
      widget.refresh();
  ...
  changeEmail() async {
    final Map map = await httpPatchEmail(Profile.sessionToken, newEmail.text);
    if (map["success"] == true) {
      setState(() {
        newEmail.clear();
